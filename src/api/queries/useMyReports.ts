...
/** 내가 제보한 가게 정렬 옵션 */
export type MyReportsSort = "LATEST" | "FAVORITE_COUNT";
 ...
 * GET /api/users/me/shops?sortBy=LATEST|FAVORITE_COUNT
 * 비로그인 상태(401 에러)에서는 에러를 던지지 않고 null을 반환합니다.
 */
export const useMyReports = (sortBy: MyReportsSort = "LATEST") => {
  return useQuery<MyReportsPageResponse | null>({
    queryKey: queryKeys.user.myReports(sortBy),
    queryFn: () =>
      get<MyReportsPageResponse | null>(
