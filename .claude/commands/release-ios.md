...
────────────────────────────────────────────
Archive + 업로드가 끝나면 "Archive 끝났어" / "업로드 끝났어" 등으로
알려주세요. App Store Connect 사이트를 자동으로 열어드리고, 위 릴리스
노트를 클립보드에 복사해 ASC에서 바로 붙여넣기 가능하도록 준비합니다.
────────────────────────────────────────────
```

### 13. Archive 완료 신호 감지 → App Store Connect 자동 오픈

사용자가 다음 중 하나의 신호를 보내면 (한국어/영어 자유):

- "Archive 끝났어" / "Archive 했어" / "Archive 완료"
- "업로드 끝났어" / "업로드 했어" / "업로드 완료"
- "ASC 열어줘" / "App Store Connect 열어줘"

다음을 **순차 실행**:

1. **릴리스 노트 클립보드 복사** (7단계 확정본):
   ...
   복사 후 "📋 릴리스 노트를 클립보드에 복사했어요. ASC에서 바로 ⌘V로 붙여넣기 가능." 메시지 출력.

2. **App Store Connect 사이트 자동 오픈** (macOS):
...
3. **마무리 안내** 출력:
   ...
   ```text
   브라우저에서 App Store Connect가 열렸어요.
   - 해당 앱 선택 → 'App Store' 탭 → '버전 또는 플랫폼 추가' (또는 기존 진행 중인 <MV> 버전 선택)
   - "이번 버전의 새로운 기능" 입력란에 ⌘V (릴리스 노트가 클립보드에 있음)
   - 빌드 섹션에서 새로 업로드한 <MV> build <BN> 선택 (처리 완료 후 노출됨, 5-30분 소요)
   - "심사를 위해 제출" 클릭
   ```

사용자가 명확한 완료 신호를 보내지 않은 상태에서는 자동 오픈 금지 (Archive 도중 실수로 트리거 방지).
